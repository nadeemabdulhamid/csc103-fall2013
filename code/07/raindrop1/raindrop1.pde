...
final int BOWL_SIZE = 40;
final color WATER_COLOR = color(150, 150, 200);
final int WIN_OR_LOSE_COUNT = 5;  
...
void draw() {
  ...
  if (gameOver) {
    ...
    moveDrop();   // make drop fall
    checkCatch(dropX, dropY); // see if it's in the bowl
  }
}
...
void checkCatch(float x, float y) {
  if (inBowl(x, y, bowlX)) {
    bowlFill = bowlFill + 1;
    resetDrop();
  }
...
boolean inBowl(float x, float y, float bx) {
  return dist(x, y, bx, GROUND_LEVEL - BOWL_SIZE/2) < BOWL_SIZE;
}

// update drop speed and location
void moveDrop() {
  dropSpeed = dropSpeed + GRAVITY;
  dropY = dropY + dropSpeed;
  if (dropY > GROUND_LEVEL) {
